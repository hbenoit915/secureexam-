  ...
  try {
    ...
    if (method === "GET" && action === "list") {
      ...
      for (const b of blobs) {
        const exam = await s.get(b.key, { type: "json" });
    ...
    if (method === "GET" && action === "get") {
      ...
      const exam = await store().get("exam-" + id, { type: "json" });
    ...
    // PATCH — toggle open/closed status
    ...
    console.error("exams function error:", err);
